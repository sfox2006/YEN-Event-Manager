...
function renderDashboard() {
  ...
  app.innerHTML = `${configNotice()}
    ...
    <section class="panel"><div class="panel-header"><h2>Next meetings</h2><a href="#/meetings">View all meetings</a></div>
      ${upcomingMeetings.length ? meetingTable(upcomingMeetings.slice(0, 5), false) : `<div class="empty-state"><h2>No meetings scheduled</h2><p>Add an executive or external organisation meeting to keep everyone aligned.</p><a class="btn btn-primary" href="#/meetings">Go to meetings</a></div>`}
    </section>`;
...
function renderMeetings() {
  ...
  const today = new Date().toISOString().slice(0, 10);
  app.innerHTML = `${configNotice()}<header class="page-header"><div><h1>Meetings</h1><p>Plan executive meetings and meetings with other organisations.</p></div><button class="btn btn-primary" id="add-meeting">+ Add meeting</button></header>
    ...
    <div class="toolbar meeting-toolbar"><label>Search<input id="meeting-search" type="search" placeholder="Search meetings"></label><label>Meeting type<select id="meeting-type-filter"><option value="">All meeting types</option>${optionList(STATUSES.meetingType)}</select></label><label>Organisation<select id="meeting-organisation-filter"><option value="">All organisations</option>${organisations.map(organisation => `<option value="${organisation.organisation_id}">${escapeHtml(organisation.organisation_name)}</option>`).join('')}</select></label><label>Organiser<select id="meeting-organiser-filter"><option value="">All organisers</option>${(state.bootstrap.committee || []).map(member => `<option value="${member.member_id}">${escapeHtml(member.name)}</option>`).join('')}</select></label><label>Status<select id="meeting-status-filter"><option value="">All statuses</option>${optionList(STATUSES.meeting)}</select></label></div>
  ...
  const update = () => {
    ...
    const organisation = document.querySelector('#meeting-organisation-filter').value;
    ...
    const filtered = meetings.filter(meeting => meetingBucket(meeting) === state.meetingFilter && `${meeting.meeting_name || ''} ${meeting.location || ''} ${meeting.external_organisation || ''} ${meeting.attendees || ''} ${meeting.agenda || ''}`.toLowerCase().includes(search) && (!type || meeting.meeting_type === type) && (!organisation || meeting.organisation_id === organisation) && (!organiser || meeting.organiser_member_id === organiser) && (!status || meeting.status === status));
  ...
  ['#meeting-search', '#meeting-type-filter', '#meeting-organisation-filter', '#meeting-organiser-filter', '#meeting-status-filter'].forEach(selector => document.querySelector(selector).addEventListener('input', update));
  update();
}
...
function meetingTable(meetings, includeActions = true) {
  ...
  return `<div class="table-wrap"><table class="responsive"><thead><tr><th>Meeting</th><th>Date & time</th><th>Organisation</th><th>Location / link</th><th>Organiser</th><th>Status</th>${includeActions ? '<th>Actions</th>' : ''}</tr></thead><tbody>${sorted.map(meeting => {
    const organiser = members.find(member => member.member_id === meeting.organiser_member_id);
    const organisation = organisations.find(row => row.organisation_id === meeting.organisation_id);
    const organisationName = organisation?.organisation_name || meeting.external_organisation || (meeting.meeting_type === 'Executive meeting' ? 'YEN Executive' : 'Not set');
...
function openMeetingDialog(record = {}) {
  ...
  const members = state.bootstrap.committee || [];
  const organisations = state.bootstrap.organisations || [];
  dialog.innerHTML = `<form method="dialog"><div class="dialog-header"><h2>${record.meeting_id ? 'Edit meeting' : 'Add meeting'}</h2><button class="icon-btn" value="cancel" aria-label="Close">✕</button></div><div class="dialog-body"><p class="subtle">Use this for YEN Executive meetings or planned meetings with another organisation. Meetings are separate from Events.</p><div class="form-grid cols-3"><label class="field-span">Meeting title<input name="meeting_name" required value="${escapeHtml(record.meeting_name)}" placeholder="YEN Executive monthly meeting"></label><label>Meeting type<select name="meeting_type">${optionList(STATUSES.meetingType, record.meeting_type || 'Executive meeting')}</select></label><label>Status<select name="status">${optionList(STATUSES.meeting, record.status || 'Planned')}</select></label><label>YEN organiser<select name="organiser_member_id"><option value="">Unassigned</option>${members.map(member => `<option value="${member.member_id}" ${member.member_id === record.organiser_member_id ? 'selected' : ''}>${escapeHtml(member.name)}${String(member.active).toLowerCase() === 'false' ? ' (inactive)' : ''}</option>`).join('')}</select></label><label>Date<input name="date" type="date" value="${escapeHtml(record.date)}"></label><label>Start time<input name="start_time" type="time" value="${escapeHtml(record.start_time)}"></label><label>End time<input name="end_time" type="time" value="${escapeHtml(record.end_time)}"></label><label class="field-span">Organisation meeting with<select name="organisation_id"><option value="">None / YEN Executive only</option>${organisations.map(organisation => `<option value="${organisation.organisation_id}" ${organisation.organisation_id === record.organisation_id ? 'selected' : ''}>${escapeHtml(organisation.organisation_name)}</option>`).join('')}</select></label><label class="field-span">Other organisation name<input name="external_organisation" value="${escapeHtml(record.external_organisation)}" placeholder="Use when the organisation is not in the directory"></label><label class="field-span">Location<input name="location" value="${escapeHtml(record.location)}" placeholder="Room name, building or Online"></label><label class="field-span">Online meeting link<input name="meeting_link" type="url" value="${escapeHtml(record.meeting_link)}" placeholder="https://…"></label><label class="field-span">Expected attendees<input name="attendees" value="${escapeHtml(record.attendees)}" placeholder="Executive members and external attendees"></label><label class="field-span">Purpose and agenda<textarea name="agenda" placeholder="Why the meeting is planned and topics to discuss">${escapeHtml(record.agenda)}</textarea></label><label class="field-span">Decisions and actions<textarea name="notes" placeholder="Record decisions, assigned actions and follow-up">${escapeHtml(record.notes)}</textarea></label></div><div class="form-actions"><span class="save-state"></span><button class="btn btn-secondary" value="cancel">Cancel</button><button class="btn btn-primary" value="save">Save meeting</button></div></div></form>`;
